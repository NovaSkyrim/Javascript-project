...
const getCoefficientWeights = () => {
  ...
  const coeffWeight = 1 - (1 / 60) * minutes;
  const weightNextHour = 1 - coeffWeight;

  return [coeffWeight, weightNextHour];
};
...
const getCurrentWaitTime = (hours, minutes) => {
  ...
  return (
    historicalDataLastWeek.find((entry) => entry.hour === adjustedHour)
      ?.waitTime || 0
  );
};
...
const main = async () => {
  ...
  console.log(
    `Le temps d'attente actuel à ${hours}h${minutes.toString().padStart(2, '0')} est de ${currentHourWaitTime} minutes.`
  );

  const targetHourString = await new Promise((resolve) => {
    readline.question(
      "Dans combien d'heures voulez-vous prédire le temps d'attente ? ",
      resolve
    );
  });

  const x = Number.parseInt(targetHourString, 10);
  if (Number.isNaN(x) || x < 0 || hours + x > 23) {
    console.error(
      "Veuillez saisir un entier positif pour représenter le nombre d'heures. L'heure de prédiction doit rester dans la plage valide (avant 23h)"
    );
    readline.close();
    return;
  }

  const waitTimePrediction = predictWaitTime(
    x,
    currentHourWaitTime,
    coefficientLists
  );
  ...
  console.log(
    `Le temps d'attente prédit dans ${x} heures est d'environ ${waitTimePrediction} minutes.`
  );
